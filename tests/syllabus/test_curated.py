...
def test_providers_sentence_introducible_per_ask_defaults_to_five_and_round_trips(tmp_path):
    ...
    assert curated.ProvidersConfig().sentence_introducible_per_ask == 5
    path = tmp_path / "providers.yaml"
    path.write_text(yaml.safe_dump(_providers(sentence_introducible_per_ask=3)))
    cfg = curated.load_providers_config(path)
    assert cfg.sentence_introducible_per_ask == 3
    curated.save_providers_config(path, cfg)
...
def test_providers_sentence_introducible_per_ask_rejects_zero(tmp_path):
    ...
    with pytest.raises(curated.CuratedValidationError,
                       match="providers.sentence_introducible_per_ask"):
        curated.load_providers_config(path)


def test_providers_sentence_introducible_per_ask_rejects_a_non_integer(tmp_path):
    path = tmp_path / "providers.yaml"
    path.write_text(yaml.safe_dump(_providers(sentence_introducible_per_ask="5")))
    with pytest.raises(curated.CuratedValidationError,
                       match="providers.sentence_introducible_per_ask"):
        curated.load_providers_config(path)
