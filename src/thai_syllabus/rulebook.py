...
SCENE_FIT_RUBRIC = (
    ...
    "production card the target word is blanked and the picture must help "
    "supply it. Judge it as a memory cue, not as a depiction. Pass when a "
    "learner who knows the sentence would take this as its picture at a "
    "glance, by any route: a literal scene, a fragment, a symbol, a "
    "consequence, a moment before or after the event, a thing standing "
    "for the action. Count what the picture implies as well as what it "
    "shows: a woman in a shop mirror holding a dress is trying it on. It "
    "must point at what is distinctive in this sentence, above all what "
    "the target word contributes, more than at the topic in general (a "
    "cup of tea evokes fifty sentences; a lump falling into tea evokes "
    "this one). Given the picture and the sentence with the target word "
    "blanked, the missing word should be recoverable. A detail the "
    "sentence adds need not appear; a prominent detail that suggests a "
    "different sentence, or contradicts this one, fails it. In "
    "`evidence`, say which element of the picture carries the link, or "
    "why none does. If it fails, give a `suggestion`: a phrase describing "
    "a picture that would cue this sentence.\n\n"
