...
class _DbMediaIndex:
    ...
    member's own current-best recording, so rendition/synthetic still sees
    provenance to warn on; pair/rendition-required and Syllabus.gaps()
    read `rendition()` instead, which takes no fallback, so the pair
    stays a need for the run to source. (rendition/mixed-speakers, which
    once read this same provenance to warn, is retired spec 1 section 4
    r10: the rendition attempt's own one-speaker intersection holds by
    construction.)
    """
    db: SyllabusDb
