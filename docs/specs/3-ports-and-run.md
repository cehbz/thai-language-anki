...
Revision 50, proposed 2026-09-20 against principles r6 and architecture
...
- r40 2026-09-17: one grapheme pass per run, after the sentence attempt and before the phrase and adjudication asks: every consonant of the repo inventory (data/thai_consonants.yaml) the deck lacks becomes a Grapheme row with its acrophonic keyword Word (matched in the vocabulary by `thai`, else a closure Word whose id is the slug of the table's gloss) and its recited-name Word (both Targets, category `Letter names`, spec 1 r16; matched in the vocabulary by `thai` too, so a run interrupted between the three file writes re-adopts without duplicating), pronunciations seeded from the engines alone -- `engines_agree` when a second engine's whole reading matches, or when the rule tone engine settles a monosyllable's tone and no other engine read the form, else `disputed`, which the adjudication pass (r28) asks the judge about next run; the three curated files are written under the writing command, rows added and none removed (spec 2 r17); a row whose keyword does not contain its symbol, or whose form no engine reads, is logged and counted, not adopted. `RunReport.adopted_graphemes`, `adopted_words` and `adoption_skipped` are events, outside the needs identity. Evidence: the live deck has 0 graphemes and E1 is satisfied vacuously -- 734 word Reading cards compile for a learner who cannot read; the inventory is fixed knowledge, so adoption is mechanical and free, and one pass takes all of it. User approval 2026-09-17. r43: a recited name the engines cannot read as a phrase is read token by token (thaig2p reads "ปอ" and "ปลา" but not "ปอ ปลา"); a row adopted without its name word gains it on a later pass, the one curated row the pass rewrites in place.
- r41 2026-09-17: `glyph`, a Provide backend of kind picture: the alphabet-chart cell for a grapheme's recited-name Word -- the symbol in a Thai font beside the keyword's current-best picture, on a square white canvas -- drawn with Pillow, deterministic and free, provenance `glyph`, keyed `glyph:<keyword picture sha>:<symbol>`, so a keyword whose picture changes is a new key; the cell is redrawn when the need reopens (a veto of the stale cell, a rubric change) -- an automatic redraw is a follow-up; a name word whose keyword has no picture has no cell and its need waits. `sources_for_need` gives that need the roster `("glyph",)` and every other need its kind's roster, read alike by the run's attempt loop, queue()/queued() and the review server's exhausted(); `glyph` is never on SOURCES["picture"]. The query is the symbol, not a drafted phrase, so the phrase ask never sees the need. A cell the judge fails leaves the name word without a Production card until the learner supplies or rates a picture (the chart-cell rubric clause is a follow-up); a learner direction cannot change a cell and is logged as such. providers.yaml gains `glyph: {font}`. Evidence: F6's grapheme card shows the keyword's picture, and the recited name's Production card needs a cue of its own -- the alphabet chart every Thai child learns from is that cue, and it is composed from artifacts the deck already holds, so searching a corpus for it would be spending money on the wrong picture. User approval 2026-09-17.
...
| glyph | picture (the alphabet-chart cell for a grapheme's recited-name Word: the symbol in the configured Thai font beside the keyword's current-best picture, square white canvas, a thin rule between; drawn with Pillow into the media store, the item carrying its sha, provenance `glyph`, licence `generated`, origin the symbol) | glyph:KEYWORD_PICTURE_SHA:SYMBOL | free; no network | never: deterministic, so the same pair is the same bytes; a keyword whose picture changes is a new key, and a name word whose keyword has no picture is not attempted (§5: the need waits) |
...
Next run, after resolve, each verdict is checked against the engines
(thaig2p, tltk; the rule tone engine on a monosyllable's tone) and, only
when no engine reading matches, against the dictionary oracle (r50:
English Wiktionary's generated IPA, one `DictionaryKey` row per form on
the record, read before the wire): a match on segments and vowel length
with the tone equal or settled by the tone engine corroborates the word
as `adjudicated` and the run
...
`quotas.wiktionary.min_interval_seconds` (1) paces the dictionary oracle
(r50); `wiktionary.contact`, optional, is appended to its user agent and
